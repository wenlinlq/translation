...
export const TTS_VOICE_OPTIONS = [
  ...
  { id: 4105, label: '度灵儿（臻品，英语推荐）', langs: ['en', 'fra', 'de', 'spa'] },
...
function buildTtsUrl(text, token, lan, per, options = {}) {
  ...
  url.searchParams.set('spd', String(options.spd ?? process.env.BAIDU_TTS_SPD ?? 5));
  url.searchParams.set('pit', String(options.pit ?? process.env.BAIDU_TTS_PIT ?? 5));
  url.searchParams.set('vol', String(options.vol ?? process.env.BAIDU_TTS_VOL ?? 8));
 ...
 * 百度在线语音合成，保存为 mp3
 */
export async function synthesizeSpeech(text, lang, outputPath, apiKey, secretKey, ttsPer) {
  ...
  throw new Error(`${lastError}（可尝试在 .env 设置 BAIDU_TTS_PER=0 使用基础音库）`);
}
